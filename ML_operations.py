...
def create_dataframe(input_raster_dir, output_csv, exclude_columns=(), search_by='*.tif', skip_processing=False):
    """
...
def split_train_test_ratio(predictor_csv, pred_attr='Subsidence', test_size=0.3, random_state=0,
...
def build_ML_classifier(predictor_csv, modeldir, model='RF', load_model=False,
    ...
    if accuracy:
        Classification_accuracy(y_test, y_pred, classifier, predictor_csv, save, accuracy_dir, cm_name,
...
def Classification_accuracy(y_test, y_pred, classifier, predictor_csv, save=True,
...
def create_prediction_raster(predictors_dir, model, yearlist=[2013, 2019], search_by='*.tif',
                             ...
                             exclude_columns=(), pred_attr='Subsidence', ref_raster=referenceraster2,
                             prediction_keyword='RF'):
    """
    ...
    pred_attr : Variable name which will be predicted. Defaults to 'Subsidence_G5_L5'.
    ref_raster : Reference global raster file. Default set to 0.02 deg resoultion global raster.
    prediction_keyword : Keyword added to final prediction raster name.
    ...
    mosaic_rasters(continent_prediction_raster_dir, prediction_raster_dir,
                   raster_name=prediction_keyword + '_prediction_' + str(yearlist[0]) + '_' + str(yearlist[1])+'.tif')
